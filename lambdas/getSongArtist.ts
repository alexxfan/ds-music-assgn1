...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  try {
    ...
    if ("stageName" in queryParams) { 
      commandInput = {
        ...commandInput,
        IndexName: "stageNameIx", 
        KeyConditionExpression: "songId = :m and begins_with(stageName, :s) ",
        ExpressionAttributeValues: {
          ":m": songId,
          ":s": queryParams.stageName,
        },
      };
    } else if ("artistName" in queryParams) { 
      commandInput = {
        ...commandInput,
        KeyConditionExpression: "songId = :m and begins_with(artistName, :a) ",
        ExpressionAttributeValues: {
          ":m": songId,
      ...
      commandInput = {
        ...commandInput,
        KeyConditionExpression: "songId = :m",
        ExpressionAttributeValues: {
          ":m": songId,
        },
      };
